...
async def process_frame(sid, frame_data, timestamp, analysis_mode):
    ...
    try:
        ...
        # Áp dụng model AI ở đây
        # --------------------------------------
        # Ví dụ:
        # results = ai_model.predict(frame)
        # text = post_process_results(results, mode=analysis_mode)
        # confidence = calculate_confidence(results)
        # --------------------------------------
        
        # Hiện tại đang dùng kết quả mẫu
...
async def translate_video(request: TranslationRequest = Body(...)):
    """
    Dịch video ngôn ngữ ký hiệu thành văn bản
    """
    # Kiểm tra dữ liệu đầu vào
    if not request.video_data and not request.video_url:
        raise HTTPException(status_code=400, detail="Bạn phải cung cấp video_data hoặc video_url")
    
    # Mô phỏng thời gian xử lý
    processing_time = random.uniform(0.5, 2.0)
    time.sleep(0.5)  # Mô phỏng độ trễ
    
    # Lấy kết quả theo chế độ phân tích
    mode = request.mode.lower()
    if mode not in ["character", "word", "sentence"]:
        mode = "word"  # Chế độ mặc định
    ...
    return {
        ...
        "video_duration": random.uniform(1.0, 10.0)  # Mô phỏng thời lượng video
    }
...
async def upload_and_translate(file: UploadFile = File(...), mode: str = Form("word")):
    """
    Tải lên và dịch tệp video
    """
    # Kiểm tra tệp
    if not file.filename.lower().endswith(('.mp4', '.webm', '.mov')):
        raise HTTPException(status_code=400, detail="Chỉ hỗ trợ định dạng video MP4, WebM, hoặc MOV")
    
    # Mô phỏng xử lý
    processing_time = random.uniform(1.0, 3.0)
    time.sleep(0.5)  # Mô phỏng độ trễ
    
    # Lấy kết quả cho chế độ phân tích
    mode = mode.lower()
    if mode not in ["character", "word", "sentence"]:
        mode = "word"  # Chế độ mặc định
...
async def get_translation_modes():
    ...
    """
    return [
        {"id": "character", "name": "Phân tích ký tự", "description": "Phát hiện từng ký tự riêng lẻ"},
        {"id": "word", "name": "Phân tích từ", "description": "Phát hiện từ hoàn chỉnh"},
        {"id": "sentence", "name": "Phân tích câu", "description": "Phát hiện và kết hợp thành câu hoàn chỉnh"}
    ]
